...
async function testDirectCharacterConsistency() {
  ...
  try {
    ...
    const content = [
      ...
      {
        type: 'text' as const,
                 text: `I'm showing you 5 generated images from a storyboard featuring a young boy explorer and his crow companion in a magical forest setting.
...
1) Analyze the surrounding forest environment elements:
   - Forest setting consistency (ancient/lush forest with tall trees)
   - Lighting conditions (natural sunlight filtering through canopy)
   - Atmosphere and mood (magical, serene, mystical)
   - Background elements (tree types, foliage, moss, forest floor)
   - Color palette (greens, browns, natural forest colors)
   - Art style consistency (Ghibli-esque animated style)
   - Terrain consistency (forest paths, ground coverage)

2) Look for environmental drift - where the forest setting appears different across scenes
...
- 0.0: Complete inconsistency - elements unrecognizable between images
...
- Forest Environment: ancient trees, lush greenery, magical atmosphere`
      }
    ];
    ...
    console.log(`Reason: ${result.object.info.reason}\n`);
     ...
     if (result.object.info.environmentConsistency) {
       console.log('🌲 [ENVIRONMENT ANALYSIS]');
       console.log('=' .repeat(50));
       const envConsistency = result.object.info.environmentConsistency;
       console.log(`Overall Environment Consistency: ${envConsistency.consistent ? '✅ YES' : '❌ NO'}`);
       console.log(`Environment Score: ${envConsistency.score.toFixed(3)} / 1.000`);
       console.log(`Environment Percentage: ${(envConsistency.score * 100).toFixed(1)}%`);

       if (envConsistency.elements && envConsistency.elements.length > 0) {
         console.log('\nEnvironment Elements:');
         envConsistency.elements.forEach((element: any, index: number) => {
           console.log(`  ${index + 1}. ${element.element}`);
           console.log(`     ✓ Consistent: ${element.consistent ? '✅ YES' : '❌ NO'}`);
           console.log(`     📝 Notes: ${element.notes}`);
         });
       }

       if (envConsistency.issues && envConsistency.issues.length > 0) {
         console.log('\nEnvironment Issues:');
         envConsistency.issues.forEach((issue: string, index: number) => {
           console.log(`  • ${issue}`);
         });
       }
       console.log();
       ...
       result.object.info.perImageAnalysis.forEach((imageAnalysis: any) => {
         ...
         if (imageAnalysis.environmentNotes) {
           console.log(`   Environment: ${imageAnalysis.environmentNotes}`);
         }
